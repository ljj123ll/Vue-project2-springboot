<!-- <template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.deptName" placeholder="部门名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
...
<script>
import { fetchList, createDept, updateDept } from '@/api/dept'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        deptName: [{ required: true, message: '未输入部门名称', trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    // 没有用上的修改数据 对应于前面的tree页面来写的 可以对其中的注释进行参考
    handleUpdate(row) {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'deptName']
        const filterVal = ['id', 'deptName']
...
<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.title" placeholder="部门名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <!-- 根据id进行升降处理 -->
      <!-- listQuery.sort与 item.key进行绑定从而改变listQuery中的参数-->
      <el-select v-model="listQuery.importance" placeholder="部门绩效" clearable style="width: 120px" class="filter-item">
 ...
 -->
    ...
    >
      ...
      <el-table-column label="部门主管" width="110px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="部门绩效" width="80px">
        <template slot-scope="{row}">
...
-->
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        <!--新增的name-->
        <el-form-item label="部门主管" prop="name">
...
<script>
// 从api中引入与后台交互的请求方法
import { fetchList, fetchPv, createArticle, updateArticle } from '@/api/dept'
// 引入水波纹
...
const calendarTypeOptions = [
  { key: '工作', display_name: '主工部' },
  { key: '政治', display_name: '行政部' },
  { key: '财务', display_name: '财务部' },
  { key: '后勤', display_name: '后勤部' }
]
